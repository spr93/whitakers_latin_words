...
package body WORD_PACKAGE is
   ...
   procedure RUN_UNIQUES
     (S       : in String; UNIQUE_FOUND : out Boolean; PA : in out PARSE_ARRAY;
   ...
   begin
      ...
      while UNQL /= null loop
         ...
         if EQU (ST, Lower_Case (UNQL.STEM)) then
            PA_LAST      := PA_LAST + 1;
            PA (PA_LAST) :=
              (UNQL.STEM, (UNQL.QUAL, 0, NULL_ENDING_RECORD, X, X), UNIQUE,
               UNQL.MNPC);
   ...
   is
      --  Tries all possible inflections against the input word in S and
      --  constructs a STEM_LIST of those that survive SL
      use LEL_SECTION_IO;
   ...
   begin
      ...
      --  Add all of these to list of possible ending records since the blank
      --  ending agrees with everything PACK/PRON have no blank endings
      if ((RESTRICTION /= PACK_ONLY) and (RESTRICTION /= QU_PRON_ONLY))
        and then (WORD'LENGTH <= MAX_STEM_SIZE)
      then
         for I in BELF (0, ' ') .. BELL (0, ' ') loop
            PR :=
              (WORD & NULL_STEM_TYPE (LENGTH_OF_WORD + 1 .. STEM_TYPE'LENGTH),
         ...
         SA (LENGTH_OF_WORD) :=
           PR.STEM;  --  Is always a possibility (null ending)
...
--TEXT_IO.PUT_LINE("#######################################################");
                  ...
                  if STEM_LENGTH <= MAX_STEM_SIZE
                  then  --  Reject too long words
                     --  Check if LEL IR agrees with PDL IR
                     --  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                     PR :=
                       (WORD (WORD'FIRST .. STEM_LENGTH) &
                     ...
                     SA (STEM_LENGTH) :=
                       PR.STEM;    --  Gets set dozens of times
   ...
   begin
      ...
      DICT_IO.Open
        (DICT_FILE (D_K), DICT_IO.In_File,
         ADD_FILE_NAME_EXTENSION
           (DICT_FILE_NAME, DICTIONARY_KIND'IMAGE (D_K)));
      LOAD_INDICES_FROM_INDX_FILE
        (ADD_FILE_NAME_EXTENSION (INDX_FILE_NAME, DICTIONARY_KIND'IMAGE (D_K)),
         D_K);
      ...
      begin                    -- Since this addresses corner case and harmless exception, handle exceptions silently
         LAST_MNPC :=
           DICT_IO.Size
             (DICT_FILE
                (GENERAL));  -- to avoid introducing new issues with via this fix
      exception
         when others =>
            LAST_MNPC := NULL_MNPC;
   ...
   procedure DICTIONARY_SEARCH
     (SSA : STEM_ARRAY_TYPE; PREFIX : PREFIX_ITEM; SUFFIX : SUFFIX_ITEM;
      D_K : DICTIONARY_KIND; RESTRICTION : DICT_RESTRICTION := REGULAR)
   is
      --  Prepares a PDL list of possible dictionary hits
      -- Search a dictionary (D_K) looking for all stems that match any of the
      -- stems that are physically possible with Latin inflections
      use STEM_IO;
      ...
      begin
         case RESTRICTION is
            when REGULAR =>
               if not
                 (DS.PART.POFS = PACK or
                  (DS.PART.POFS = PRON and then (DS.PART.PRON.DECL.WHICH = 1)))
               then
            ...
            when QU_PRON_ONLY =>
               if DS.PART.POFS = PRON and then (DS.PART.PRON.DECL.WHICH = 1)
               then
                  PDL_INDEX       := PDL_INDEX + 1;
                  PDL (PDL_INDEX) := PRUNED_DICTIONARY_ITEM'(DS, D_K);
               end if;
   ...
   begin
      --  Now go through the dictionary list DL for the first letters and make
      --  a reduced dictionary list PDL
      ...
      if INDEX_FIRST > 0 and then INDEX_FIRST <= INDEX_LAST then
         ...
         for K in SSA'RANGE loop
            if TRIM (SSA (K))'LENGTH > 1 then
               --  This may be checking for 0 and 1 letter SSAs which are done
               --  elsewhere
               --TEXT_IO.PUT(INTEGER'IMAGE(K) & "  SSA(K) =>" );
               --TEXT_IO.PUT_LINE(SSA(K));

               if D_K = LOCAL
               then    --  Special processing for unordered DICT.LOC
                  for J in J1 .. J2
                  loop       --  Sweep exaustively through the scope
...
--TEXT_IO.PUT_LINE("Leaving DICTIONARY_SEARCH PDL_INDEX = " & INTEGER'IMAGE(PDL_INDEX));
   ...
   begin
      ...
      --  BDL is always used, so it is loaded initially and not called from
      --  disk Check all stems of the dictionary entry against the reduced
      --  stems
      ...
      --  Now there is only one blank stem (2 of to_be), but need not always be
      --  so

      --  Determine if there is a blank stem (SC = ' ') Prepare for the
      --  posibility that one stem is short but there are others
      FC := ' ';
      ...
      if FC /= ' ' then
         for I in 2 .. BDL_LAST loop
            --  Check all stems of the dictionary entry against the reduced
            --  stems
   ...
   begin
      if Upper_Case (C) = 'L' and then not CL_Arguments (ENGLISH_ONLY) then
   ...
   is
      ...
      begin
         ...
         loop
            ...
            begin
               ...
               --  sloppy One problem is that it can mix up some of the order
               --  of PREFIX, XXX, LOC I ought to do this for every set of
               --  results from different approaches not just in one fell
               --  swoop at the end !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               INNER_LOOP :
               for I in 1 .. SL_LAST - 1 loop
                  if SL (I + 1) /= NULL_PARSE_RECORD then
                     if (SL (I + 1).MNPC < SL (I).MNPC)
                       or else
                       (SL (I + 1).MNPC = SL (I).MNPC
                        and then SL (I + 1).IR.ENDING.SIZE <
                          SL (I).IR.ENDING.SIZE)
                       or else
                       (SL (I + 1).MNPC = SL (I).MNPC
                        and then SL (I + 1).IR.ENDING.SIZE =
      ...
      procedure ARRAY_STEMS
        (SX : in SAL; PA : in out PARSE_ARRAY; PA_LAST : in out Integer)
      is
         SL  : SAL          := SX;
         ...
         if SL (1) = NULL_PARSE_RECORD then
            ...
            for I in SL'RANGE loop
               if SL (I) /= NULL_PARSE_RECORD then
                  ...
                  begin
                     if SL (I) <= OPR
                     then       --  Get rid of duplicates, if ORDER is OK
                        --PUT('-'); PUT(SL(I)); NEW_LINE;
                        null;
                     else
                        PA_LAST := PA_LAST + 1;
...
--TEXT_IO.PUT_LINE("At the end of ARRAY_STEMS");
      ...
      is
         ...
         begin
            if RIGHT = LEFT or else (LEFT = PACK and RIGHT = PRON)
              or else RIGHT = X
            then
               return True;
            else
         ...
         begin
            if
              (RIGHT = LEFT or else (RIGHT = C and LEFT /= N)
               or else (RIGHT = X))
            then
               return True;
            else
      ...
      begin
         ...
         for J in 1 .. PDL_INDEX loop
            ...
            if (SUFFIX = NULL_SUFFIX_ITEM)
            then  --  No suffix working, fall through
            --PUT_LINE("No SUFFIX in REDUCE - Fall through to PREFIX check ");
               null;
            elsif
              (PDL_P = N
               and then PDL_PART.N.DECL = (9, 8)) or  --  No suffix for
            ...
            then  --  abbreviations
               ...
               -- vellum demonstrates how this is problematic because the
               -- vellum result suppresses a much more likely set of results
               -- (v/b trick => bellum)

            else                      --  There is SUFFIX, see if it agrees with PDL
               if PDL_P <= SUFFIX.ENTR.ROOT
                 and then     --  Does SUFFIX agree in ROOT

                 ((PDL_KEY <= SUFFIX.ENTR.ROOT_KEY)
                  or else
                  ((PDL_KEY = 0)
                   and then ((PDL_P = N) or (PDL_P = ADJ) or (PDL_P = V))
                   and then
                   ((SUFFIX.ENTR.ROOT_KEY = 1) or (SUFFIX.ENTR.ROOT_KEY = 2))))
               then
                  --PUT_LINE("HIT HIT HIT HIT HIT HIT HIT HIT HIT     SUFFIX SUFFIX    in REDUCE");
                  case SUFFIX.ENTR.TARGET.POFS
                  is      --  Transform PDL_PART to TARGET
                     when N =>
            ...
            if (PREFIX = NULL_PREFIX_ITEM)
            then      --  No PREFIX, drop through
               --PUT_LINE("No PREFIX in REDUCE - Fall through to MATCHing ");
               null;
            elsif
              (PDL_P = N
               and then PDL_PART.N.DECL = (9, 8)) or  --  No prefix for
              (PDL_P = ADJ
               and then PDL_PART.ADJ.DECL = (9, 8)) or --  abbreviations

              (PDL_P = INTERJ or PDL_P = CONJ)
            then  --  or INTERJ or CONJ
...
--PUT(PDL_P); PUT(" <= "); PUT(PREFIX.ENTR.ROOT); PUT("       OR"); NEW_LINE;
               ...
               if (PDL_P = PREFIX.ENTR.ROOT) or    --  = ROOT
                 (PDL_PART.POFS = PREFIX.ENTR.ROOT)
               then  --  or part mod by suf
...
--                       end if;
            ...
            --  SUFFIX and PREFIX either agree or don't exist (agrees with
            --  everything)
      ...
      --PUT_LINE(ADD_SUFFIX(ADD_PREFIX(PDL(J).DS.STEM, PREFIX), SUFFIX) & "|");
            ...
            LS :=
              LEN (ADD_SUFFIX (ADD_PREFIX (PDL (J).DS.STEM, PREFIX), SUFFIX));
            ...
            for I in SL'RANGE loop
               exit when SL (I) = NULL_PARSE_RECORD;
--
--  TEXT_IO.PUT("SL(I) "); PARSE_RECORD_IO.PUT(SL(I));
--  TEXT_IO.NEW_LINE; TEXT_IO.PUT(" PDL ");
--  TEXT_IO.PUT(PDL(J).DS.STEM); PART_ENTRY_IO.PUT(PDL(J).DS.PART);
--  Text_IO.PUT(INTEGER(PDL(J).DS.KEY)'image); TEXT_IO.NEW_LINE;
--  TEXT_IO.PUT("LS "); TEXT_IO.PUT(INTEGER'IMAGE(LS)); TEXT_IO.PUT("
--  LEN SL(I).STEM "); TEXT_IO.PUT_LINE(INTEGER'IMAGE(LEN(SL(I).STEM)));

               if LS = LEN (SL (I).STEM) then

                  --  Scan through the whole unreduced stem list Single out
                  --  those stems that match (pruned) dictionary entries
                  ...
                  if
                    (((PDL_KEY <= SL (I).IR.KEY))
                     ...
                     ((PDL_KEY = 0)
                      and then
                      (((PDL_P = N) or (PDL_P = ADJ) or (PDL_P = V))
                       and then ((SL (I).IR.KEY = 1) or (SL (I).IR.KEY = 2)))))
                    and then   --  and KEY
...
--TEXT_IO.PUT_LINE("#######################   PDL - SL  MATCH   ############");
                     ...
                     if
                       (PDL_PART.POFS = N
                        and then PDL_PART.N.DECL <= SL (I).IR.QUAL.N.DECL
                        and then PDL_PART.N.GENDER <= SL (I).IR.QUAL.N.GENDER)
                     then
                        --TEXT_IO.PUT_LINE(" HIT  N     ");
                        --  Need to transfer the gender of the noun dictionary
                        --  item
                        M       := M + 1;
                        SXX (M) :=
                          (STEM => SUBTRACT_PREFIX (SL (I).STEM, PREFIX),
                           IR   =>
                             (QUAL =>
                                (POFS => N,
                                 N    =>
                                   (PDL_PART.N.DECL, SL (I).IR.QUAL.N.CS,
                                    SL (I).IR.QUAL.N.NUMBER,
                                    PDL_PART.N.GENDER)),
                              KEY => SL (I).IR.KEY, ENDING => SL (I).IR.ENDING,
                              AGE => SL (I).IR.AGE, FREQ => SL (I).IR.FREQ),
                           D_K => PDL (J).D_K, MNPC => MNPC_PART);

                     elsif
                       (PDL_PART.POFS = PRON
                        and then PDL_PART.PRON.DECL <=
                          SL (I).IR.QUAL.PRON.DECL)
                     then
                        --PUT(" HIT  PRON  ");
                        --  Need to transfer the kind of the pronoun dictionary
                        --  item
                        M       := M + 1;
                        SXX (M) :=
                          (STEM => SUBTRACT_PREFIX (SL (I).STEM, PREFIX),
                           IR   =>
                             (QUAL =>
                                (POFS => PRON,
                                 PRON =>
                                   (PDL_PART.PRON.DECL, SL (I).IR.QUAL.PRON.CS,
                     ...
                     elsif (PDL_PART.POFS = ADJ)
                       and then (PDL_PART.ADJ.DECL <= SL (I).IR.QUAL.ADJ.DECL)
                       and then
                     ...
                     then
                        ...
                        if PDL_PART.ADJ.CO in POS .. SUPER then
                           ...
                           --  Otherwise, the entry is X, generate a CO from
                           --  KEY
                          ...
                          (STEM => SUBTRACT_PREFIX (SL (I).STEM, PREFIX),
                           IR   =>
                             (QUAL =>
                                (POFS => ADJ,
                                 ADJ  =>
                                   (PDL_PART.ADJ.DECL, SL (I).IR.QUAL.ADJ.CS,
                                    SL (I).IR.QUAL.ADJ.NUMBER,
                                    SL (I).IR.QUAL.ADJ.GENDER, COM)),
                              KEY => SL (I).IR.KEY, ENDING => SL (I).IR.ENDING,
                              AGE => SL (I).IR.AGE, FREQ => SL (I).IR.FREQ),
                           D_K => PDL (J).D_K, MNPC => MNPC_PART);

                     elsif (PDL_PART.POFS = NUM)
                       and then (PDL_PART.NUM.DECL <= SL (I).IR.QUAL.NUM.DECL)
                       and then (PDL_KEY = SL (I).IR.KEY)
                     then
                        --PUT(" HIT  NUM    ");
                        if PDL_PART.NUM.SORT = X then
                           ...
                           --  Otherwise, the dictionary entry has a unique CO,
                           --  use it
                          ...
                          (STEM => SUBTRACT_PREFIX (SL (I).STEM, PREFIX),
                           IR   =>
                             (QUAL =>
                                (POFS => NUM,
                                 NUM  =>
                                   (PDL_PART.NUM.DECL, SL (I).IR.QUAL.NUM.CS,
                                    SL (I).IR.QUAL.NUM.NUMBER,
                                    SL (I).IR.QUAL.NUM.GENDER, NUM_SORT)),
                              KEY => SL (I).IR.KEY, ENDING => SL (I).IR.ENDING,
                              AGE => SL (I).IR.AGE, FREQ => SL (I).IR.FREQ),
                           D_K => PDL (J).D_K, MNPC => MNPC_PART);

                     elsif (PDL_PART.POFS = ADV)
                       and then
                     ...
                     then
                        ...
                        if PDL_PART.ADV.CO in POS .. SUPER then
                           ...
                           --  The entry is X and we need to generate a COMP
                           --  from the KEY
                        ...
                        if SL (I).IR.QUAL.POFS = V
                          and then (PDL_PART.V.CON <= SL (I).IR.QUAL.V.CON)
                        then
                           --TEXT_IO.PUT(" HIT  V     ");
                           M       := M + 1;
                           SXX (M) :=
                        ...
                        elsif SL (I).IR.QUAL.POFS = VPAR
                          and then (PDL_PART.V.CON <= SL (I).IR.QUAL.VPAR.CON)
                        then
                           --PUT(" HIT  VPAR  ");
                           M       := M + 1;
                           SXX (M) :=
                             (STEM => SUBTRACT_PREFIX (SL (I).STEM, PREFIX),
                              IR   =>
                                (QUAL =>
                                   (POFS => VPAR,
                                    VPAR =>
                                      (PDL_PART.V.CON, SL (I).IR.QUAL.VPAR.CS,
                        ...
                        elsif SL (I).IR.QUAL.POFS = SUPINE
                          and then
                     ...
                     elsif PDL_PART.POFS = PREP
                       and then PDL_PART.PREP.OBJ = SL (I).IR.QUAL.PREP.OBJ
                     then
                        --PUT(" HIT  PREP  ");
                        M       := M + 1;
                        SXX (M) :=
      ...
      procedure APPLY_PREFIX
        (SA  : in     STEM_ARRAY_TYPE; SUFFIX : in SUFFIX_ITEM; SX : in SAL;
         SXX : in out SAL; PA : in out PARSE_ARRAY; PA_LAST : in out Integer)
      is
         --  Worry about the stem changing re-cipio from capio Correspondence
         --  of parts, need EFF for VPAR The prefixes should be ordered with
         --  the longest/most likely first
         SSA : STEM_ARRAY;
         L   : Integer := 0;
         --use TEXT_IO;
...
--PUT(NUMBER_OF_PREFIXES); PUT(INTEGER(SA'LENGTH)); PUT(SA'LAST); NEW_LINE;
            for I in 1 .. NUMBER_OF_PREFIXES
            loop       --  Loop through PREFIXES
               L := 0;
               for J in SA'RANGE
               loop                  --  Loop through stem array
--PUT("J = "); PUT(J); PUT("   SA(J) = "); PUT(SA(J)); NEW_LINE;
                  if (SA (J) (1) = PREFIXES (I).FIX (1))
                  then  --  Cuts down a little -- do better
...
--PUT_LINE("Hit on prefix  " & PREFIXES(I).FIX);
            --PUT("I = "); PUT(I); PUT("  "); PUT(PREFIXES(I).FIX); PUT("  ");
                        ...
                        L :=
                          L +
                          1;            --  We have a hit, make new stem array item
                        SSA (L) :=
                          HEAD
                            (SUBTRACT_PREFIX (SA (J), PREFIXES (I)),
               ...
               if L > 0
               then                        --  There has been a prefix hit
                  SEARCH_DICTIONARIES
                    (SSA (1 .. L),      --  So run new dictionary search
                     PREFIXES (I), SUFFIX);

                  if PDL_INDEX /= 0
                  then                  --  Dict search was successful
...
--PUT_LINE("REDUCE_STEM_LIST being called from APPLY_PREFIX  ----  SUFFIX = "
                  --& SUFFIX.FIX);
                     ...
                     if SXX (1) /= NULL_PARSE_RECORD
                     then   --  There is reduced stem result
                        PA_LAST :=
                          PA_LAST +
                          1;        --  So add prefix line to parse array
                        PA (PA_LAST).IR :=
                          ((PREFIX, NULL_PREFIX_RECORD), 0, NULL_ENDING_RECORD,
                           X, X);
                        PA (PA_LAST).STEM :=
      ...
      procedure APPLY_SUFFIX
        (SA : in     STEM_ARRAY_TYPE; SX : in SAL; SXX : in out SAL;
         ...
         for I in 1 .. NUMBER_OF_SUFFIXES loop       --  Loop through SUFFIXES
            ...
            for J in SA'RANGE loop                  --  Loop through stem array
               ...
               then
                  ...
                  -- TEXT_IO.PUT("I = "); PUT(I); PUT(" ");
                  -- PUT(SUFFIXES(I).FIX); PUT(" "); TEXT_IO.PUT("J =
                  -- "); PUT(J); PUT(" "); PUT(SA(J)); NEW_LINE; DEBUG
                  ...
                  SSA (L) :=
                    HEAD
                      (SUBTRACT_SUFFIX (SA (J), SUFFIXES (I)),
                       MAX_STEM_SIZE);  --  And that has prefix subtracted to match dict
            ...
            if L > 0
            then                        --  There has been a suffix hit
               SEARCH_DICTIONARIES
                 (SSA (1 .. L), NULL_PREFIX_ITEM,
                  SUFFIXES
                    (I));     --  So run new dictionary search
               ...
               --TEXT_IO.PUT_LINE("REDUCE_STEM_LIST called from APPLY_SUFFIX");
                  ...
                  then    --  There is reduced stem result
                     PA_LAST :=
                       PA_LAST +
                       1;        --  So add suffix line to parse array
                     --   TEXT_IO.PUT_LINE("REDUCE_STEM_LIST is not null so add suffix to parse array");
                     PA (PA_LAST).IR :=
                       ((SUFFIX, NULL_SUFFIX_RECORD), 0, NULL_ENDING_RECORD, X,
                        X);
                     PA (PA_LAST).STEM :=
...
--                    TEXT_IO.PUT("SUFFIX_HIT = "); PUT(SUFFIXES(I).FIX); NEW_LINE;
                  ...
                  then    --  There is reduced stem result
                     ...
                     PA (PA_LAST).MNPC :=
                       DICT_IO.Count (SUFFIXES (SUFFIX_HIT).MNPC);
                     --  TEXT_IO.PUT("SUFFIX MNPC ");
                     --  PUT(SUFFIXES(SUFFIX_HIT).MNPC); NEW_LINE;
                     PA (PA_LAST).D_K := ADDONS;
      ...
      procedure PRUNE_STEMS
        (INPUT_WORD : String; SX : in SAL; SXX : in out SAL)
      is
         J : Integer := 0;
         --SXX : SAL;
...
--TEXT_IO.PUT_LINE("PRUNE_STEMS   checking (not)    DO_ONLY_FIXES   = " & BOOLEAN'IMAGE(WORDS_MDEV(DO_ONLY_FIXES)));
         if not WORDS_MDEV (DO_ONLY_FIXES)
         then   --  Just bypass main dictionary search
--TEXT_IO.PUT_LINE("Calling SEARCH_DICTIONARIES from PRUNE_STEMS   ---  General case");

            SEARCH_DICTIONARIES
              (SSA (1 .. SSA_MAX), NULL_PREFIX_ITEM, NULL_SUFFIX_ITEM);
...
--TEXT_IO.PUT_LINE("PRUNE_STEMS   below  NOT DO_ONLY_FIXES  PA_LAST = " & INTEGER'IMAGE(PA_LAST));
         ...
         if
           (
            ((PA_LAST = 0) and            --  No Uniques or Syncope

             (PDL_INDEX =
              0))  --)   and then    --  No dictionary match
         or
            WORDS_MDEV (DO_FIXES_ANYWAY))
           and then WORDS_MODE (DO_FIXES)
         then
...
--TEXT_IO.PUT_LINE(" PDL_INDEX = 0     after straight search   ------  So APPLY_SUFFIX  PA_LAST = " & INTEGER'IMAGE(PA_LAST));
            ...
            if SXX (1) = NULL_PARSE_RECORD
            then        --  We could not find a match with suffix
               APPLY_PREFIX
                 (SSA (1 .. SSA_MAX), NULL_SUFFIX_ITEM, SX, SXX, PA, PA_LAST);
            end if;
            --------------
            if SXX (1) = NULL_PARSE_RECORD
            then        --  We could not find a match with suffix
...
--TEXT_IO.PUT_LINE(" PDL_INDEX = 0     after suffix search  -----  So APPLY_PREFIX by itself  PA_LAST = " & INTEGER'IMAGE(PA_LAST));
                  ...
                  APPLY_PREFIX
                    (SSA (1 .. SSA_MAX), NULL_SUFFIX_ITEM, SX, SXX, PA,
                     PA_LAST);
...
--TEXT_IO.PUT_LINE("End of PRUNE_STEMS   PA_LAST = " & INTEGER'IMAGE(PA_LAST));
      ...
      procedure PROCESS_PACKONS (INPUT_WORD : String; KEY : STEM_KEY_TYPE := 0)
      is
...
--              end "<=";
      ...
      begin
         ...
         OVER_PACKONS :
         for K in PACKONS'RANGE
         loop    -- Do whole set, more than one may apply
...
--PUT_LINE("FOR_EACH_PACKON   PACKON_LENGTH = "& INTEGER'IMAGE(PACKON_LENGTH));
            ...
            --PUT_LINE("PROCESS_PACKONS Hit on PACKON    " & PACKONS(K).TACK);
                  ...
                  if PACKONS (K).TACK (1 .. 3) = "dam" and LAST_OF_WORD = 'n'
                  then
                     WORD (WORD'LAST) :=
                       'm';   --  Takes care of the m - > n shift with dam
                     LAST_OF_WORD := 'm';
               --PUT_LINE("PACKON = dam   and LAST_OF_WORD = n    => " & WORD);
                  ...
                  M := 0;
                  ON_INFLECTS :
                  for Z in reverse 1 .. MIN (6, LENGTH_OF_WORD)
                  loop   --  optimum for qu-pronouns
      --PUT("ON_INFLECTS  Z = "); PUT(Z); PUT("  "); PUT(WORD(1..Z)); NEW_LINE;
                     if PELL (Z, LAST_OF_WORD) > 0
                     then   --  Any possible inflections at all
                        for I in
                          PELF (Z, LAST_OF_WORD) .. PELL (Z, LAST_OF_WORD)
                        loop
   --PUT("+");PUT(LEL(I)); PUT(WORD'LAST); PUT(WORD(WORD'LAST-Z+1..WORD'LAST));
         ...
         --PUT_LINE("SSA(1) in PACKONS from real  INFLECTS ->" & SSA(1) & '|');
                  ...
                  PDL_INDEX := 0;
                  SEARCH_DICTIONARIES
                    (SSA (1 .. 1), NULL_PREFIX_ITEM, NULL_SUFFIX_ITEM,
                     PACK_ONLY);
                  --  Now have a PDL, scan for agreement

                  PDL_LOOP :
                  for J in 1 .. PDL_INDEX
                  loop  --  Go through all dictionary hits to see
                     --PUT_LINE("PACKON  PDL_INDEX  "); PUT(PDL(J).DS.STEM); PUT(PDL(J).DS.PART); NEW_LINE;
                     --  M used here wher I is used in REDUCE, maybe make
                     --  consistent
                     M := 1;
                     SL_LOOP :
                     while SL (M) /= NULL_PARSE_RECORD
                     loop  --  Over all inflection hits
                        --  if this stem is possible
                        --  call up the meaning to check for "(w/-"
                        DICT_IO.Set_Index
                          (DICT_FILE (PDL (J).D_K), PDL (J).DS.MNPC);
                        DICT_IO.Read (DICT_FILE (PDL (J).D_K), DE);
                        MEAN := DE.MEAN;

                        if
                          (TRIM (MEAN) (1 .. 4) = "(w/-"
                           and then  --  Does attached PACKON agree
                                ...
                                (STEM => SL (M).STEM,
                                 IR   =>
                                   (QUAL =>
                                    ...
                                    AGE    => SL (M).IR.AGE,
                                    FREQ   => SL (M).IR.FREQ),
                                 D_K => PDL (J).D_K, MNPC => PDL (J).DS.MNPC);
                              --end if;
                           end if;
                        end if;
                        M := M + 1;
      ...
      procedure PROCESS_QU_PRONOUNS
        (INPUT_WORD : String; QKEY : STEM_KEY_TYPE := 0)
      is
...
--TEXT_IO.PUT_LINE("PROCESS_QU_PRONOUNS   " & INPUT_WORD);
         ...
         --  No blank endings in qu-pronouns
         LEL_SECTION_IO.Read (INFLECTIONS_SECTIONS_FILE, LEL, 4);

         --  M used here while I is used in REDUCE, maybe make consistent
         M := 0;
         ON_INFLECTS :
         for Z in reverse 1 .. MIN (4, LENGTH_OF_WORD)
         loop     --  optimized for qu-pronouns
      --PUT("ON_INFLECTS  "); PUT(Z); PUT("  "); PUT(LAST_OF_WORD); NEW_LINE;
            ...
            then   --  Any possible inflections at all
               for I in PELF (Z, LAST_OF_WORD) .. PELL (Z, LAST_OF_WORD) loop
                  ...
                  if (Z <= LENGTH_OF_WORD) and then LEL (I).KEY = QKEY
                    and then EQU
                      (LEL (I).ENDING.SUF (1 .. Z),
      ...
      --PUT_LINE("INFLECTS HIT --------------------------------------------");
                     ...
                     STEM_LENGTH := WORD'LENGTH - Z;
                     PR          :=
                       (HEAD (WORD (WORD'FIRST .. STEM_LENGTH), MAX_STEM_SIZE),
                        LEL (I), DEFAULT_DICTIONARY_KIND, NULL_MNPC);
                     M      := M + 1;
                     SL (M) := PR;
         ...
         PDL_INDEX := 0;
         SEARCH_DICTIONARIES
           (SSA (1 .. 1), NULL_PREFIX_ITEM, NULL_SUFFIX_ITEM, QU_PRON_ONLY);
         --  Now have a PDL, scan for agreement
                          ...
                          (POFS => PRON,
                           PRON =>
                             (PDL (J).DS.PART.PRON.DECL,
                              ...
                              SL (M).IR.QUAL.PRON.GENDER,
                              SL (M).IR.QUAL.PRON.KIND)),
                        KEY => SL (M).IR.KEY, ENDING => SL (M).IR.ENDING,
                        AGE => SL (M).IR.AGE, FREQ => SL (M).IR.FREQ),
                     D_K => PDL (J).D_K, MNPC => PDL (J).DS.MNPC);
               end if;
               M := M + 1;
      ...
      procedure TRY_TACKONS (INPUT_WORD : String) is
         ...
         START_OF_LOOP    : Integer          :=
           5;    --  4 enclitics     --  Hard number  !!!!!!!!!!!!!!!
...
--                  TEXT_IO.NEW_LINE;
   ...
   --PART_OF_SPEECH_TYPE_IO.PUT(TACKONS(I).ENTR.BASE.PART); TEXT_IO.NEW_LINE;
                     ...
                     --PUT("TACKON MNPC  "); PUT( TACKONS(I).MNPC); NEW_LINE;
                        ...
                        while J >= ENTERING_PA_LAST + 1
                        loop        --  Sweep backwards over PA
                           --  Sweeping up inapplicable fixes, although we only
                           --  have TACKONs for X or PRON or ADJ - so far and
                           --  there are no fixes for PRON - so far
...
--TEXT_IO.NEW_LINE;
            ...
            --PUT("TACKON PRON found HIT "); PUT( TACKONS(I).TACK); NEW_LINE;
                                 ...
                                 when ADJ =>
                                    --  Forgo all checks, even on DECL of ADJ
                                    TACKON_HIT := True;
                                    TACKON_ON  := True;
      ...
      begin       --  QU
         ...
         for I in 1 .. NUMBER_OF_TICKONS + 1 loop
            ...
            begin
               ...
               if
                 (I =
                  NUMBER_OF_TICKONS + 1) or else  --  The prefix is a TICKON

                 (Q_WORD /= INPUT_WORD)
               then            --  and it matches the start of INPUT_WORD
...
--TEXT_IO.PUT_LINE("ADDING TICKON PA    " & TICKONS(I).FIX);
                     PA_LAST :=
                       ...
                       1;        --  So add prefix line to parse array
                     PA (PA_LAST).STEM :=
                       HEAD (TICKONS (I).FIX, MAX_STEM_SIZE);
                     PA (PA_LAST).IR :=
                       ((PREFIX, NULL_PREFIX_RECORD), 0, NULL_ENDING_RECORD, X,
                        X);
                     PA (PA_LAST).D_K  := ADDONS;
                  ...
                  if Q_WORD'LENGTH >= 3
                    and then   --  qui is shortest QU_PRON
                  ...
                  then
                     ...
                     elsif Q_WORD (Q_WORD'FIRST .. Q_WORD'FIRST + 1) = "cu"
                     then
                        QKEY := 2;
                        PROCESS_QU_PRONOUNS (Q_WORD, QKEY);
                     end if;
                     ...
                     then    --  If did not find a PACKON
                        if Q_WORD (Q_WORD'FIRST .. Q_WORD'FIRST + 1) = "qu"
                        then
                     ...
                     if INPUT_WORD (INPUT_WORD'FIRST .. INPUT_WORD'FIRST + 4) =
                       "aliqu"
                     then
                        PROCESS_QU_PRONOUNS (INPUT_WORD, 1);
                     elsif INPUT_WORD
                         (INPUT_WORD'FIRST .. INPUT_WORD'FIRST + 4) =
                       "alicu"
                     then
...
--TEXT_IO.PUT_LINE("Out WORD  INPUT = " & INPUT_WORD & "   PA_LAST = " & INTEGER'IMAGE(PA_LAST));
   ...
   exception
      when Storage_Error =>
         Text_IO.Put_Line
           (Text_IO.Standard_Output,
            "STORAGE_ERROR exception in WORD while processing =>" & RAW_WORD);
         PA_LAST := PA_SAVE;
   ...
   begin                                  --  Initializing WORD_PACKAGE
      ...
      LEL_SECTION_IO.Open
        (INFLECTIONS_SECTIONS_FILE, LEL_SECTION_IO.In_File,
      ...
      begin
         ...
         LOAD_DICTIONARY
           (DICT_LOC, ADD_FILE_NAME_EXTENSION (DICTIONARY_FILE_NAME, "LOCAL"));
...
--TEXT_IO.PUT_LINE("BDL loaded");
      if not
        (DICTIONARY_AVAILABLE (GENERAL) or DICTIONARY_AVAILABLE (SPECIAL) or
