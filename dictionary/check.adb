...
procedure CHECK is
   ...
   begin
      if (Lower_Case (C) = 'a') or (Lower_Case (C) = 'e') or
        (Lower_Case (C) = 'i') or (Lower_Case (C) = 'o') or
        (Lower_Case (C) = 'u')
      then
         return True;
      else
   ...
   begin
      ...
      if PT.POFS = N then
         ...
         if
           (BK (STS (1)) or BK (STS (2)) or not BK (STS (3)) or
            not BK (STS (4)))
           and then not (PT.N.DECL = (9, 9) or PT.N.DECL = (9, 8))
         then  --  Undeclined
            PROB ("    EXPECTED  exactly 2  NOUN STEMS");
         end if;

         --  Check that the stems are the same when expected
         if PT.N.DECL = (1, 1) or PT.N.DECL = (2, 1) or PT.N.DECL = (2, 2) or
           PT.N.DECL = (4, 1) or PT.N.DECL = (5, 1)
         then
            if STS (1) /= STS (2)
              and then ((STS (1) /= ZZZ_STEM) and (STS (2) /= ZZZ_STEM))
            then
         ...
         if PT.N.DECL = (1, 2)
           and then ((STS (1) /= ZZZ_STEM) or (STS (2) /= ZZZ_STEM))
         then
            if LEN (STS (1)) >= 3
              and then STS (1) (LEN (STS (1)) .. LEN (STS (1))) /= "r"
              and then STS (2) (LEN (STS (2)) .. LEN (STS (2))) /= "r"
            then
         ...
         if PT.N.DECL = (3, 1) then
            if
              (LEN (STS (1)) >= 3
               and then STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "er")
              and then (PT.N.GENDER /= M and PT.N.GENDER /= C)
            then
         ...
         --  Check er -> er
         if PT.N.DECL = (3, 1) and PT.N.GENDER = M then
            if
              (LEN (STS (1)) >= 3
               and then STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "er")
              and then
         ...
         if PT.N.DECL = (3, 1) then
            if
              (LEN (STS (1)) >= 3
               and then STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "or")
              and then PT.N.GENDER /= M
            then
         ...
         if PT.N.DECL = (3, 1)
           and then (STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "o")
           and then PT.N.GENDER /= M
         then
            if (LEN (STS (1)) >= 3)
              and then
              ((STS (1) (LEN (STS (1)) - 2 .. LEN (STS (1))) /= "do") and
               ...
               (STS (1) (LEN (STS (1)) - 2 .. LEN (STS (1))) /= "io"))
              and then PT.N.GENDER /= F
            then
         ...
         --  Check io -> ion
         if PT.N.DECL = (3, 1) and PT.N.GENDER = F then
            if LEN (STS (1)) >= 3
              and then STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "io"
              and then STS (2) (LEN (STS (2)) - 2 .. LEN (STS (2))) /= "ion"
            then
         ...
         if PT.N.DECL = (3, 1) and PT.N.GENDER = F and LEN (STS (1)) >= 3 then
            if STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "do"
              and then STS (2) (LEN (STS (2)) - 2 .. LEN (STS (2))) /= "din"
            then
               PROB ("    EXPECTED  do -> din  NOUN  3 1 STEMS");

            elsif  STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "do"
               and then STS (2) (LEN (STS (2)) - 2 .. LEN (STS (2))) = "din"
            then
            if STS (1) (1 .. LEN (STS (1)) - 1) /=
              STS (2) (1 .. LEN (STS (2)) - 2)
         ...
         if PT.N.DECL = (3, 1) and PT.N.GENDER = F then
            if LEN (STS (1)) >= 3
              and then STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "as"
              and then
         ...
         if PT.N.DECL.WHICH = 3 then
            if (LEN (STS (1)) >= 3)
              and then (STS (1) (LEN (STS (1)) .. LEN (STS (1))) = "a")
              and then
         ...
         then
            if LEN (STS (1)) >= 3
              ...
              (STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "es" or
               STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "is")
              and then STS (1) (1 .. LEN (STS (1)) - 2) =
                STS (2) (1 .. LEN (STS (2)))
            then
         ...
         if PT.N.DECL = (3, 3) then
            if (LEN (STS (1)) >= 3)
              and then (STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "es")
            then
         ...
         --  Check al/e  -> I-stem
         if PT.N.DECL.WHICH = 3 and (PT.N.GENDER = N) then
            if LEN (STS (1)) >= 3
              and then
         ...
         --  Can only check consistency if more than one stem, CO /= COMP |
         --  SUPER
         if (PT.ADJ.CO = POS or PT.ADJ.CO = X) and
           (PT.ADJ.DECL /= (9, 9) and PT.ADJ.DECL /= (9, 8))
         then
...
((STS (1) /= ZZZ_STEM) and (STS (2) /= ZZZ_STEM))
            ...
            if PT.ADJ.DECL = (3, 1) then
               if LEN (STS (1)) >= 3
                 and then STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "ns"
                 and then STS (2) (LEN (STS (2)) - 1 .. LEN (STS (2))) /= "nt"
               then
         ...
         if PT.ADJ.DECL.WHICH = 9 and PT.ADJ.CO /= POS then
            PROB ("    EXPECTED  ADJ 9 to be POS");
         end if;

         --  Check that ADJ 9 has 1 stem
         if (PT.ADJ.DECL.WHICH = 9) and (STS (2) /= NULL_STEM_TYPE) then
            PROB ("    EXPECTED  ADJ 9 have just 1 stem");
         end if;

         --  Check that there are two and only two stems if POS
         if PT.ADJ.CO = POS and PT.ADJ.DECL /= (9, 9) and PT.ADJ.DECL /= (9, 8)
         then
            if (STS (3) /= NULL_STEM_TYPE or STS (4) /= NULL_STEM_TYPE) then
         ...
         if PT.ADJ.CO = X then
            if (STS (3) /= NULL_STEM_TYPE and STS (3) /= ZZZ_STEM) then
         ...
         if PT.ADJ.CO = X then
            if ((LEN (STS (3)) > 3) and (LEN (STS (4)) > 4))
              and then
              ((STS (3) /= NULL_STEM_TYPE and STS (3) /= ZZZ_STEM)
               and then (STS (4) /= NULL_STEM_TYPE and STS (4) /= ZZZ_STEM))
            then
               if (STS (3) (LEN (STS (3)) - 3 .. LEN (STS (3))) = "cili") then
                  if (STS (4) (LEN (STS (4)) - 4 .. LEN (STS (4))) /= "cilli")
                  then
                     PROB
                       ("    EXPECTED  'cil' ADJ  STEM 4 to end in 'cilli'");
                  end if;
               elsif (STS (3) (LEN (STS (3)) - 3 .. LEN (STS (3))) = "mili")
               then
                  if (STS (4) (LEN (STS (4)) - 4 .. LEN (STS (4))) /= "milli")
                  then
                     PROB
                       ("    EXPECTED  'mil' ADJ  STEM 4 to end in 'milli'");
                  end if;
               elsif (STS (3) (LEN (STS (3)) - 1 .. LEN (STS (3))) = "ri") then
                  if (STS (4) (LEN (STS (4)) - 2 .. LEN (STS (4))) /= "rri")
                  then
                     PROB ("    EXPECTED  'r' ADJ  STEM 4 to end in 'rri'");
                  end if;
               elsif STS (4) (LEN (STS (4)) - 3 .. LEN (STS (4))) /= "issi"
               then
         ...
         if ((PT.ADJ.CO = COMP) or (PT.ADJ.CO = SUPER))
           and then (PT.ADJ.DECL /= (0, 0))
         then
         ...
         if (PT.ADV.CO = POS or PT.ADV.CO = X) then
            ...
            if PT.ADV.CO = POS then
               if (STS (2) /= NULL_STEM_TYPE or STS (3) /= NULL_STEM_TYPE) then
         ...
         if PT.ADV.CO = X then
            if (STS (2) /= NULL_STEM_TYPE and STS (2) /= ZZZ_STEM)
              and then (STS (2) (LEN (STS (2)) - 2 .. LEN (STS (2))) /= "ius")
            then
         ...
         if PT.ADV.CO = X then
            if (STS (3) /= NULL_STEM_TYPE and STS (3) /= ZZZ_STEM)
              and then (STS (3) (LEN (STS (3)) - 2 .. LEN (STS (3))) /= "ime")
            then
         ...
         if PT.ADV.CO = X then
            if ((LEN (STS (2)) > 4) and (LEN (STS (3)) > 6))
              and then
              ((STS (2) /= NULL_STEM_TYPE and STS (2) /= ZZZ_STEM)
               and then (STS (3) /= NULL_STEM_TYPE and STS (3) /= ZZZ_STEM))
            then
          if LEN (STS (2)) > 5 then
              if (STS (2) (LEN (STS (2)) - 5 .. LEN (STS (2))) = "cilius")
               then
                  if
                    (STS (3) (LEN (STS (3)) - 6 .. LEN (STS (3))) /= "cillime")
                  then
                     PROB
                       ("    EXPECTED  'cil' ADV  STEM 3 to end in 'cillime'");
                  end if;
               elsif (STS (2) (LEN (STS (2)) - 5 .. LEN (STS (2))) = "milius")
               then
                  if
                    (STS (3) (LEN (STS (3)) - 6 .. LEN (STS (3))) /= "millime")
                  then
                     PROB
                       ("    EXPECTED  'mil' ADV  STEM 3 to end in 'millime'");
                  end if;
               elsif (STS (2) (LEN (STS (2)) - 3 .. LEN (STS (2))) = "rius")
               then
                  if (STS (3) (LEN (STS (3)) - 4 .. LEN (STS (3))) /= "rrime")
                  then
                     PROB ("    EXPECTED  'r' ADV  STEM 3 to end in 'rrime'");
                  end if;
               elsif STS (3) (LEN (STS (3)) - 5 .. LEN (STS (3))) /= "issime"
               then
         ...
         if PT.V.CON = (9, 9) then
            if
              (STS (2) /= NULL_STEM_TYPE or STS (3) /= NULL_STEM_TYPE or
            ...
            --  Check to see no third verb stem has lingering 'i'
             if LEN(STS(3)) > 2 and then
                ( (STS (3) (LEN (STS (3))) = 'i') and (PT.V.CON /= (6, 1)) ) then
            ...
            if PT.V.CON.WHICH < 5 and STS (1) (1 .. 3) /= "zzz" then
               if
                 (PT.V.CON /= (3, 1) and PT.V.CON /= (3, 2) and
                  PT.V.CON /= (3, 4))
               then
                  if STS (1) /= STS (2) then
                     PROB ("    EXPECTED IDENTICAL VERB 1 & 2 STEMS");
                  end if;
               elsif PT.V.CON.WHICH = 2
                 and then (STS (1) (LEN (STS (1))) = 'e')
               then
                  PROB ("    EXPECTED (2, X) not to have -e 1st stem");
               elsif PT.V.CON = (3, 1) and then STS (1) /= STS (2)
                 and then
                 (STS (1) (1 .. LEN (STS (1))) /=
                  STS (2) (1 .. LEN (STS (2))) & 'i')
               then
               ...
               elsif PT.V.CON = (3, 2) and then ( Len(STS(1)) > 2 and Len(Sts(2)) > 3 ) then
                  if
                    ((STS (1) (LEN (STS (1)) - 2 .. LEN (STS (1))) /= "fer") or
                     (STS (2) (LEN (STS (2)) - 3 .. LEN (STS (2))) /=
                      "ferr")) or
                    (STS (1) (1 .. LEN (STS (1)) - 3) /=
                     STS (2) (1 .. LEN (STS (2)) - 4))
                  then
            ...
            if PT.V.CON = (1, 1)
              and then (STS (3) /= ZZZ_STEM and STS (4) /= ZZZ_STEM)
            then

               if LEN (STS (3)) >= 3
                 and then (STS (3) (LEN (STS (3)) - 1 .. LEN (STS (3))) = "av")
               then
          ...
          elsif Len(STS(1)) > 1 and then
            (STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "st")
               then  --  sto bad
            ...
            if PT.V.CON = (3, 1)
              and then
              (STS (1) /= ZZZ_STEM and then STS (2) /= ZZZ_STEM
               and then STS (3) /= ZZZ_STEM and then STS (4) /= ZZZ_STEM)
            then
               if LEN (STS (1)) >= 4
                 and then (STS (3) (LEN (STS (3)) - 1 .. LEN (STS (3))) = "ec")
               then
                  ...
                  then
                     PROB
                       ("    EXPECTED  (3, 1) 3/4  feci, fec, fec, fact  VERB STEMS");
                  end if;
                  ...
                  then
                     PROB
                       ("    EXPECTED  (3, 1) 3/4  fici, fic, fec, fect  VERB STEMS");
                  end if;
               end if;
            elsif PT.V.CON = (3, 1) and then STS (4) /= ZZZ_STEM then
               if LEN (STS (3)) >= 3
                 and then (STS (3) (LEN (STS (3)) .. LEN (STS (3))) = "x")
               then
                  if STS (3) (LEN (STS (3)) - 1 .. LEN (STS (3))) = "nx" then
                     if not
                       ((STS (4) (LEN (STS (4)) - 2 .. LEN (STS (4))) =
                         "nct" and
                         STS (3) (1 .. LEN (STS (3)) - 1) =
                           STS (4) (1 .. LEN (STS (4)) - 3)) or
                        (STS (4) (LEN (STS (4)) - 1 .. LEN (STS (4))) =
                         "ct" and
                         STS (3) (1 .. LEN (STS (3)) - 1) =
                           STS (4) (1 .. LEN (STS (4)) - 3)))
                     then
                        PROB
                          ("    EXPECTED  (3, 1) 3/4  nx -> (n)ct   VERB STEMS");
                     end if;
                  elsif STS (3) (LEN (STS (3)) - 2 .. LEN (STS (3))) = "fix"
                  then
                     if
                       (STS (3) (1 .. LEN (STS (3))) /=
                        STS (4) (1 .. LEN (STS (4))))
                    ...
                    and then STS (3) (LEN (STS (3)) - 3 .. LEN (STS (3))) =
                      "flex"
                  then
                     if
                       (STS (3) (1 .. LEN (STS (3))) /=
                  ...
                  then
                     ...
                     then
                        PROB
                          ("    EXPECTED  (3, 1) 3/4  flux -> flux  VERB STEMS");
                     end if;
            ...
            if DE.PART.V.KIND = DEP and then STS (3) (1 .. 3) /= "zzz" then
      ...
      --  Catch others
      if ( (PT.POFS = N) or (PT.POFS = ADJ) )
        and then STS(2) /= NULL_STEM_TYPE
        then
         if LEN (STS (1)) >= 3
           and then STS (1) (LEN (STS (1)) .. LEN (STS (1))) = "u"
           and then IS_VOWEL (STS (2) (LEN (STS (2)) - 1))
         then
            PROB ("    CHECK for terminal u or v      ");
         end if;
         if LEN (STS (1)) >= 3

           and then STS (1) (LEN (STS (1)) .. LEN (STS (1))) = "v"
           and then not IS_VOWEL (STS (2) (LEN (STS (2)) - 1))
         then
            PROB ("    CHECK for terminal u or v      ");
         end if;
      end if;

      if (PT.POFS = V) and then (PT.V.CON = (2, 1)) then
         if (LEN (STS (1)) >= 3)
           and then (STS (1) (LEN (STS (1)) - 1 .. LEN (STS (1))) = "pl")
           and then (STS (3) (LEN (STS (3)) - 1 .. LEN (STS (3))) /= "ev")
         then
   ...
   exception
      when Catch_Me : others =>
         Put_Line ("Unhandled exception at " & Integer'Image (LINE_NUMBER));
...
begin
   Put_Line
     ("Takes a DICTLINE form named CHECK.IN, analyzes for possible errors, and");
   Put_Line
     ("produces a report CHECK.OUT - Remember to process CHECK.OUT from end");
   ...
   while not End_Of_File (INPUT) loop
      ...
      begin
         ...
         if S (19) /= ' ' or S (38) /= ' ' or S (57) /= ' ' or S (76) /= ' ' or
           S (102) /= ' '
         then
            NUMBER := NUMBER + 1;
            Put (OUTPUT, "LINE");
