...
--TEXT_IO.PUT_LINE("START PRINT MODIFIED QUAL " );      --  Prints the line(s) that explain each inflected form
   QUALITY_RECORD_IO.PUT(OUT_STRING, SR.IR.QUAL);        --  E.g., "V      1 1 PRES ACTIVE  IMP 2 S   "
...
--TEXT_IO.PUT_LINE("In the ADJ -> ADV kludge  Adding POS for ADV");
                     PA(PA_LAST) := (PA(J2+1).STEM,
                                            ((POFS => ADV, ADV => (CO => PA(J2+1).IR.QUAL.ADJ.CO, GENERATED => ADJADV)),
...
--TEXT_IO.PUT_LINE("PUTting MEANING");
 ...
 TEXT_IO.NEW_LINE(OUTPUT);
    ...
    procedure UNKNOWN_SEARCH(UNKNOWN       :  in STRING;
         ...
         INDEX_ON : constant STRING := Lower_Case(UNKNOWN); -- SPR:  Fixes bug that prevented unknown_search from working if ignore_unknown's were set to N, do_unknowns_only set to Y, and input had a capital letter
