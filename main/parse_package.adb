...
--TEXT_IO.NEW_LINE;
               ...
               YYY_MEANING_COUNTER := 1;
               NNN_MEANING_COUNTER := 1;
               RRR_MEANING_COUNTER := 1;
               PPP_MEANING_COUNTER := 1;
...
--TEXT_IO.PUT_LINE("In ENCLITICS after WORD NO_FIXES  LESS = " & LESS & "   PA_LAST = " & INTEGER'IMAGE(PA_LAST));
                              ...
                              if PA_LAST = 0 then
                                 ...
                                 TRY_SLURY (LESS, PA, PA_LAST);
                                 if SAVE_PA_LAST /= 0 then
                                    if (PA_LAST - 1) - SAVE_PA_LAST =
                                      SAVE_PA_LAST
                                    then
...
-- TACKONS(I).TACK);
                           if LESS /= TRY then       --  LESS is less
                              --PASS(LESS);
                              TRY_TRICKS (LESS, TRPA, TRPA_LAST);
                              ...
                              then      --  have a possible word
                                 ...
                                 TRPA (ENTERING_TRPA_LAST + 2 .. TRPA_LAST) :=
                                   TRPA
                                     (ENTERING_TRPA_LAST + 1 .. TRPA_LAST - 1);
...
--end loop;
                     ...
                     if PA_LAST = 0 then
                        TRY_SLURY (INPUT_WORD, PA, PA_LAST);
                     end if;
...
--TEXT_IO.PUT_LINE("DO_TRICKS      PA_LAST    TRPA_LAST  " & INTEGER'IMAGE(PA_LAST) & "   " & INTEGER'IMAGE(TRPA_LAST));
                        ...
                        TRY_TRICKS (INPUT_WORD, TRPA, TRPA_LAST);
                  ...
                  then    --  But PA may be killed by ALLOW in LIST_STEMS
                     ...
                     then
                        ...
                        declare
                           ...
                           COMPOUND_TENSE : INFLECTIONS_PACKAGE.TENSE_TYPE :=
                             X;
                        ...
                        begin
                                    ...
                                    loop        --  Sweep backwards to kill empty suffixes
                                       ...
                                       then
                                          ...
                                          then
                                             ...
                                             case SUM_INFO.TENSE_VOICE_MOOD
                                               .TENSE
                           ...
                           then     --  On NEXT_WORD
                              ...
                              if K = NK then      --  There was a PPL hit
                                 ...
                                 PA (PA_LAST) :=
                                   (HEAD ("PPL+" & NEXT_WORD, MAX_STEM_SIZE),
                                    ((V, (PPL_INFO.CON, COMPOUND_TVM, 0, X)),
                                     0, NULL_ENDING_RECORD, X, A),
                                    PPP, NULL_MNPC);
                                 ...
                                 begin
                                    ...
                                    loop        --  Sweep backwards to kill empty suffixes
                                       ...
                                       then
                                         ...
                                         and then PA (J).IR.QUAL.SUPINE.CS =
                                           ACC
                                       then
...
-- INTEGER'IMAGE(PA_LAST));
                  ...
                  XXX_MEANING_COUNTER := 0;
                  YYY_MEANING_COUNTER := 0;
                  NNN_MEANING_COUNTER := 0;
                  RRR_MEANING_COUNTER := 0;
                  PPP_MEANING_COUNTER := 0;
...
--  All Rights Reserved - William Armstrong Whitaker
      ...
      if METHOD = COMMAND_LINE_INPUT then
         if TRIM (COMMAND_LINE) /= "" then
            PARSE_LINE (Ada.Characters.Handling.To_Basic (COMMAND_LINE));
         end if;
         ...
         then
            ...
            if CONFIGURATION = ONLY_MEANINGS then
               PREFACE.PUT_LINE
                 ("- MEANINGS ONLY:   Inflections will not display in Latin-to-English mode");
            end if;
            if CL_Arguments (ENGLISH_ONLY) then
               PREFACE.PUT_LINE
                 ("- ENGLISH ONLY :   The program will not enter Latin-to-English mode");
            end if;
            if CL_Arguments (LATIN_ONLY) then
               PREFACE.PUT_LINE
                 ("- LATIN ONLY   :   The program will not enter English-to-Latin mode");
            end if;
            if CL_Arguments (READ_ONLY) then
               PREFACE.PUT_LINE
                 ("- READ ONLY    :   Options cannot be changed; no output to file");
            end if;
            if CL_Arguments (NO_FILES) then
               PREFACE.PUT_LINE
                 ("- NO FILES     :   The program will not load files for translation");
            end if;
         ...
         then
            ...
            PREFACE.PUT_LINE
              ("English dictionary not available.  Cannot run in English-to-Latin only mode.");
            raise GIVE_UP;
         ...
         case LANGUAGE is
            ...
            when LATIN_TO_ENGLISH =>
               PREFACE.PUT ("Input a word or line of Latin and press ENTER");
            ...
            begin                    --  Block to manipulate file of lines
               ...
               then            -- Not a blank line so L(1) (in file input)
                  ...
                  then
                    ...
                    and then not
                    (CL_Arguments (ENGLISH_ONLY) or CL_Arguments (LATIN_ONLY))
                    and then (Name (Current_Input) = Name (Standard_Input))
                  then
