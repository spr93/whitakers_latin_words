...
--
   ...
   begin
   -- The ages generated by this function differ slightly from those identified in Arabic2Roman. That's intentional. The principles are:
   --
   -- 1. When translating FROM Latin (this function), identify a specific age only in very clear cases to avoid a false sense of certainty.
   -- 2. When translating TO Latin (Arabic2Roman), discourage users from using non-canonical forms by denigrating them as archaic ("obsolete by classical period")
   --    times." Most people were (incorrectly) taught that "Roman numerals" always subtract immediately preceding smaller values.
   --    We don't want to mislead the casual student into providing a "wrong" answer to an audience that believes in strict subtractive rules.
      ...
      if S (S'Last) = 'j' or S (S'Last) = 'J' then
         ...
         return
           X; -- valid subtractive form within the range of numbers used at all times; no further tests can disambiguate
    ...
    return F;   -- medieval (see comment above)
    else
      return X;
    end if;
